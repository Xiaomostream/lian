...
class Exp_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0, show_progress=True, test_data=None, test_loader=None):
        ...
        for i, batch in enumerate(test_loader):
            ...
            if (not exported) and getattr(self.args, 'save_preds', False):
                try:
                    os.makedirs(getattr(self.args, 'pred_save_dir', './predictions'), exist_ok=True)
                    tag = getattr(self.args, 'pred_save_tag', '')
                    tag = f"_{tag}" if tag else ''
                    out_path = os.path.join(
                        getattr(self.args, 'pred_save_dir', './predictions'),
                        f"{setting}{tag}_sample.npz",
                    )
                    np.savez(
                        out_path,
                        input=batch_x.detach().cpu().numpy()[0],
                        gt=batch_y.numpy()[0],
                        pred=outputs.numpy()[0],
                        seq_len=int(getattr(self.args, 'seq_len', batch_x.shape[1])),
                        pred_len=int(getattr(self.args, 'pred_len', outputs.shape[1])),
                        data_path=str(getattr(self.args, 'data_path', '')),
                        model=str(getattr(self.args, 'model', '')),
                        model_id=str(getattr(self.args, 'model_id', '')),
                    )
                    print(f"Saved prediction sample to {out_path}")
                    exported = True
                except Exception as e:
                    print(f"Warning: failed to export prediction sample: {e}")

...
class MoiraiForecast(L.LightningModule):
    ...
    def _generate_time_id(
        ...
        if past_seq_id[..., -1].min() == past_seq_id[..., -1].max():
            past_seq_id = past_seq_id[..., 0, :]
            batch_shape = ""
        else:
    ...
    def _convert(
        ...
        past_seq_id, future_seq_id = self._generate_time_id(
            patch_size, ~past_is_pad.bool().unsqueeze(-1)
        )
            ...
            [
                repeat(
                    reduce(
                        (
                            self._patched_seq_pad(
                                patch_size, past_is_pad.min(0)[0], -1, left=True, value=1
                            )
                            == 0
                        ).int(),
                        "... (seq patch) -> ... seq",
                ...
                [
                    repeat(
                        reduce(
                            (
                                self._patched_seq_pad(
                                    patch_size, past_is_pad.min(0)[0], -1, left=True
                                )
                                == 0
                            ).int(),
                            "... (seq patch) -> ... seq",

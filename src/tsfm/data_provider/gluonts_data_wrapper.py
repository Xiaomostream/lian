...
class StandardScalingGluonTSDataset:
    ...
    def transform(self, data: Union[TrainingDataset, InputDataset]):
        ...
        for i, _ in tqdm(enumerate(out)):
            ...
            out[i]["target"] = out[i]["target"].squeeze()
        return iter(out)
...
class TorchDatasetFromGluonTSTrainingDataset(Dataset):
    def __init__(
        ...
        # self.X = torch.tensor(np.vstack([x["target"] for x in self.X]), dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_mask:
            return seq_x.T, seq_y.T, past_observed_mask.T
        else:
            return seq_x.T, seq_y.T

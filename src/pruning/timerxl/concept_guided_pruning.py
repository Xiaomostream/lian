...
class ConceptGuidedPruning:
    ...
    def __init__(
        self,
        concept_map_path: str = './concept_maps/timerxl_concept_map.pkl',
        task_vector_path: str = './task_vectors/etth1_task_vector.pkl'
    ):
        print("="*60)
        print("概念引导剪枝初始化 (TimerXL)")
        print("="*60)
    ...
    def analyze_importance_distribution(self, module_importance):
        ...
        ax1.set_title('TimerXL Module Importance Distribution', fontsize=14)
        ax1.grid(alpha=0.3)
        ...
        save_path = './visualizations/timerxl_importance_distribution.pdf'
        Path(save_path).parent.mkdir(parents=True, exist_ok=True)

...
def visualize_concept_sensitivity_heatmap(
    concept_map: dict,
    save_path: str = './visualizations/concept_sensitivity_heatmap.pdf'
):
    """
    绘制概念敏感度热图
    ...
    Args:
        concept_map: {
            'periodicity': {'encoder.layer.0': 0.85, ...},
            'trend': {...},
            ...
        }
        save_path: 保存路径
    ...
    sns.heatmap(
        ...
        yticklabels=[m.replace('backbone.', '') for m in modules],
        annot=False
    )
    
    ax.set_xlabel('Concept', fontsize=14, labelpad=10)
    ax.set_ylabel('Model Module', fontsize=14, labelpad=10)
    ax.set_title('Concept Sensitivity Heatmap', fontsize=16, pad=15)
...
def visualize_concept_bar_chart(
    ...
    top_modules = [item[0].replace('backbone.', '') for item in sorted_items[:top_k]]
    ...
    ax.set_title(f'Top-{top_k} Modules for {concept_name.capitalize()} Concept', 
                 fontsize=14, pad=10)
    ...
    if save_path is None:
        save_path = f'./visualizations/{concept_name}_top_modules.pdf'
...
def analyze_concept_map(concept_map: dict):
    ...
    for concept, sensitivity in concept_map.items():
        ...
        for name, score in sorted_items[:3]:
            print(f"    {name.replace('backbone.', '')}: {score:.4f}")
...
if __name__ == '__main__':
    ...
    if not Path(concept_map_path).exists():
        ...
        print("请先运行 02_train_concept_probe.sh")
        exit(1)
    ...
    # 生成热图
    print("\n生成可视化...")
    visualize_concept_sensitivity_heatmap(concept_map)
    
    # 为每个概念生成柱状图
    for concept in concept_map.keys():
        visualize_concept_bar_chart(concept_map, concept, top_k=10)
    
    print("\n✓ 可视化完成! 结果保存在 ./visualizations/")

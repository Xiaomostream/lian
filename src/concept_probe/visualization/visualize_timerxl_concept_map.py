...
def visualize_concept_sensitivity_heatmap(concept_map, save_path):
    ...
    ax.set_ylabel('TimerXL Module', fontsize=14, labelpad=10)
    ax.set_title('TimerXL Concept Sensitivity Heatmap', fontsize=16, pad=15)
...
def visualize_concept_bar_chart(concept_map, concept_name, top_k=8, save_path=None):
    ...
    ax.set_title(f'Top-{top_k} TimerXL Modules for {concept_name.capitalize()} Concept', fontsize=14)
    ...
    if save_path is None:
        save_path = f'./visualizations/probe_training_timerxl/{concept_name}_top_modules.pdf'
    Path(save_path).parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    if not Path(concept_map_path).exists():
        ...
        print("请先运行 bash scripts/timerxl/02_train_timerxl_concept_probe.sh")
        exit(1)
    ...
    visualize_concept_sensitivity_heatmap(
        concept_map,
        save_path='./visualizations/probe_training_timerxl/concept_sensitivity_heatmap.pdf'
    )
    
    for concept in concept_map.keys():
        visualize_concept_bar_chart(
            concept_map, concept, top_k=8,
            save_path=f'./visualizations/probe_training_timerxl/{concept}_top_modules.pdf'
        )
    
    print("\n✓ 可视化完成! 结果保存在 ./visualizations/probe_training_timerxl/")
